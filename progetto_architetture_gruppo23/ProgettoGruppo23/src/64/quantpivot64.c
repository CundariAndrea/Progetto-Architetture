...
extern void quantize_vector(const type *vec, int D, int x, int *out_idx,
                            int *out_sign);
...
void fit(params *input) {
  ...
  for (int j = 0; j < h; j++) {
    quantize_vector(&input->DS[input->P[j] * D], D, x, &pivot_idxs[j * x],
                    &pivot_signs[j * x]);
  }
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < h; j++) {
      input->index[i * h + j] = dist_approx(
          point_idx, point_sign, &pivot_idxs[j * x], &pivot_signs[j * x], x);
    }
  }
...
void predict(params *input) {
  ...
  if (input->DS == NULL) {
    printf("FATAL ERROR: input->DS is NULL (Dataset persa?)\n");
    exit(1);
  }
  if (input->Q == NULL) {
    printf("FATAL ERROR: input->Q is NULL (Query non passata?)\n");
    exit(1);
  }
  if (input->P == NULL) {
    printf("FATAL ERROR: input->P is NULL (Fit non eseguito?)\n");
    exit(1);
  }
  if (input->index == NULL) {
    printf("FATAL ERROR: input->index is NULL (Fit non eseguito?)\n");
    exit(1);
  }
  // -----------------------------
