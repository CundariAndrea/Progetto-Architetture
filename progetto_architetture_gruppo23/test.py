...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["32", "64", "64omp"],
        help="float+sse, double+avx, double+avx+openmp",
    )
    parser.add_argument(
        "-s", "--silent", action="store_true", help="modalità silenziosa"
    )
    ...
    if args.t == "32":
        quantpivot = gruppo23.quantpivot32.QuantPivot()
    elif args.t == "64":
        quantpivot = gruppo23.quantpivot64.QuantPivot()
    elif args.t == "64omp":
        quantpivot = gruppo23.quantpivot64omp.QuantPivot()
    ...
    save_ds2(
        ids,
        f"idNN_{args.t}_size-{DS.shape[0]}x{DS.shape[1]}_nq-{Q.shape[0]}.ds2",
        dtype=f"float{args.t}",
    )

    save_ds2(
        ids,
        f"distNN_{args.t}_size-{DS.shape[0]}x{DS.shape[1]}_nq-{Q.shape[0]}.ds2",
        dtype=f"float{args.t}",
    )
